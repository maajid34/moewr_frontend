...
export default function EnergyAchievementsTable() {
  ...
  const PATHS = useMemo(
    () => ["/readProjectEnergy/EnergyProject", "/api/readProjectEnergy/EnergyProject"],
    []
  );
  const ACHIEVEMENTS_PREFIXES = useMemo(
    () => ["/createAchiev", "/api/createAchiev"],
...
const handleDelete = async (projectId, index) => {
  try {
    ...
    await axios.delete(`http://localhost:3000/DeleteEnergyAchiev/${projectId}/achievements/${index}`);
  ...
  return (
    <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 absolute left-[300px] top-[100px]">
     <Link to={"/EnergyAchievementForm"}> <button className="px-5 py-2 rounded-xl bg-blue-900 text-white m-5">Add Energy Achievments</button></Link>
...
>
  <i className="fa-solid fa-trash mr-2" /> Delete
                     ...
                     <Link to={`/EnergyAchievementUpdate/${r.projectId}/${r.achIndex}`}>
  <button
