...
// // export default function SingalProjectsEnergy() {
// //   const { id } = useParams();
...
//   // NEW: Fetch Photos via your route: /ReadEnergyProjectPhoto/:id/photos
//   useEffect(() => {
//     let mounted = true;
...
//       setPhotosError(null);
//       try {
//         const res = await axios.get(`${API}/ReadEnergyProjectPhoto/${id}/photos`);
//         // Expecting { photos: [{ Image: "..." }, ...] }
//         const arr =
...
export default function SingalProjectsEnergy() {
  ...
  useEffect(() => {
    ...
    const fetchOne = async () => {
      ...
      setError(null);
      try {
        const res = await axios.get(`${API}/readProjectEnergySingal/EnergyProject/${id}`);
        const data = Array.isArray(res.data) ? res.data[0] : (res.data?.data ?? res.data);
        if (!mounted || !data) return;
