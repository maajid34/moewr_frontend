...
export default function WaterProjectFormUI() {
  ...
  const handleCreateProWater = async (e) => {
    ...
    // stakeholder text (MUST match backend keys)
    ...
    // stakeholder logos (MUST match backend keys)
    if (stackeHolder1) fd.append("stackeHolder1", stackeHolder1); // note the 'e' in stackeHolder1
    ...
    // achievements (server expects array JSON, or you can leave as string & parse server-side)
    fd.append("achievements", achievements);

    // gallery
    for (const file of projectPhotos) fd.append("photos", file);

    // DEBUG: verify what we’re sending
    for (const [k, v] of fd.entries()) {
      // Files will log as File objects; strings as text
  ...
  return (
    <div className="absolute left-[300px] top-[100px] min-h-screen w-[calc(100%-300px)] bg-slate-50 text-slate-800">
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <form className="space-y-8" method="post" encType="multipart/form-data" onSubmit={handleCreateProWater}>
          {/* ——— Basic fields (title/desc/overview) ——— */}
          {/* … keep your existing JSX … */}

          {/* Stakeholder description (overall) */}
          <input
            value={StackeholderDesc}
          ...
          <section className="grid lg:grid-cols-2 gap-6">
            <div className="rounded-2xl border bg-white shadow-sm p-5">
              ...
              <div className="mt-4 border border-dashed p-6 text-center rounded-xl">
                <label className="inline-block text-xs px-3 py-2 rounded-md bg-sky-500 text-white cursor-pointer">
                  <input
                    onChange={(e) => setstackeHolder1(e.target.files?.[0] || null)}
                    type="file"
                    name="stackeHolder1"
                    id="stackeHolder1"
                    accept="image/*"
            ...
            <div className="rounded-2xl border bg-white shadow-sm p-5">
              ...
              <div className="mt-4 border border-dashed p-6 text-center rounded-xl">
                <label className="inline-block text-xs px-3 py-2 rounded-md bg-sky-500 text-white cursor-pointer">
                  <input
                    onChange={(e) => setstakeHolder2(e.target.files?.[0] || null)}
                    type="file"
                    name="stakeHolder2"
                    id="stakeHolder2"
                    accept="image/*"
            ...
            <div className="rounded-2xl border bg-white shadow-sm p-5">
              ...
              <div className="mt-4 border border-dashed p-6 text-center rounded-xl">
                <label className="inline-block text-xs px-3 py-2 rounded-md bg-sky-500 text-white cursor-pointer">
                  <input
                    onChange={(e) => setstakeHolder3(e.target.files?.[0] || null)}
                    type="file"
                    name="stakeHolder3"
                    id="stakeHolder3"
                    accept="image/*"
            ...
            {/* Stakeholder 4 (optional) */}
            <div className="rounded-2xl border bg-white shadow-sm p-5">
              <h3 className="font-semibold">Stakeholder 4</h3>
              <div className="mt-4 border border-dashed p-6 text-center rounded-xl">
                <label className="inline-block text-xs px-3 py-2 rounded-md bg-sky-500 text-white cursor-pointer">
                  <input
                    onChange={(e) => setstakeHolder4(e.target.files?.[0] || null)}
                    type="file"
                    name="stakeHolder4"
                    id="stakeHolder4"
                    accept="image/*"
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            <button type="submit" className="rounded-xl bg-[#536878] px-5 py-2.5 text-white">
              Save Project
            </button>
          </div>
        </form>
