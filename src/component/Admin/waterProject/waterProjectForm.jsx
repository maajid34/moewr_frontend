...
export default function WaterProjectFormUI() {
  ...
  // Stakeholders (TEXT) — MUST MATCH BACKEND
  const [StackeholderDesc, setStackeholderDesc] = useState("");
  const [stack1Title, setstack1Title] = useState("");
  const [stack1desc, setstack1desc] = useState("");
  const [stack2Title, setstack2Title] = useState("");
  ...
  // Stakeholders (IMAGES) — note stackeHolder1 has an 'e'
  const [stackeHolder1, setstackeHolder1] = useState(null);
  const [stakeHolder2, setstakeHolder2] = useState(null);
  const [stakeHolder3, setstakeHolder3] = useState(null);
  const [stakeHolder4, setstakeHolder4] = useState(null);
  ...
  const handleCreateProWater = async (e) => {
    ...
    // Single images (names MUST match multer field names)
    if (coverImage) formData.append("coverImage", coverImage);
    if (objectiveImage) formData.append("objectiveImage", objectiveImage);
    if (GeographicImage) formData.append("GeographicImage", GeographicImage);
    ...
    // Stakeholders (TEXT) — EXACT keys your backend requires
    formData.append("StackeholderDesc", StackeholderDesc);
    formData.append("stack1Title", stack1Title);
    formData.append("stack1desc", stack1desc);
    formData.append("stack2Title", stack2Title);
    ...
    // Stakeholders (IMAGES) — EXACT keys
    if (stackeHolder1) formData.append("stackeHolder1", stackeHolder1); // note the 'e'
    if (stakeHolder2) formData.append("stakeHolder2", stakeHolder2);
    if (stakeHolder3) formData.append("stakeHolder3", stakeHolder3);
    if (stakeHolder4) formData.append("stakeHolder4", stakeHolder4);

    // Achievements (string; backend can parse to array if needed)
    formData.append("achievements", achievements);

    // Gallery (multiple)
    for (const file of projectPhotos) {
      formData.append("photos", file);
      // If your backend expects "photos[]", use formData.append("photos[]", file);
    }
    ...
    <div className="absolute left-[300px] top-[100px] min-h-screen w-[calc(100%-300px)] bg-slate-50 text-slate-800 [--brand:#2FA8E1] [--brand-dark:#0A7FB8] [--ink:#002B5C]">
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        >
          ...
          {/* Stakeholders section description */}
          <input
            value={StackeholderDesc}
            onChange={(t) => setStackeholderDesc(t.target.value)}
            type="text"
            ...
            <div className="rounded-2xl border bg-white shadow-sm p-5">
              ...
              <div className="mt-4 rounded-xl border border-dashed p-6 text-center">
                ...
                <input
                  ...
                  required
                  placeholder="Enter Title"
                  className="mt-3 w-full rounded-lg border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--brand)]"
                />
