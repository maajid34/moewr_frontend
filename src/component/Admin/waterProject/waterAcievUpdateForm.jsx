...
export default function WaterAchievementUpdate() {
  ...
  useEffect(() => {
    ...
    const url = `${API}/ReadSigngalWaterAchiev/${id}/achievements/${index}`;
    axios
      .get(url)
      .then((res) => {
        if (!mounted) return;
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await axios.put(`${API}/Update/${id}/achievements/${index}`, {
        title: title.trim(),
        detail: detail.trim(),
        progress: p,
      });
      setMsg("Achievement updated ✅");
      setTimeout(() => navigate("/WaterAchievementsTable"), 700);
    } catch (e) {
