...
//     await axios.delete(`${API}/DeleteWaterProject/waterProject/${id}`, {
...
//       setError(null);
//       try {
//         const res = await axios.get(`${API}/readProjectWater/waterProject`);
...
//         <Link to={"/WaterProjectForm"}><button className="py-2  px-5 mb-[20px] rounded-xl bg-blue-900 text-white">New Project</button></Link>
//         <div className="flex items-center justify-between mb-4">
//           <h3 className="font-semibold">Water Projects List</h3>
...
//                         {/* geographic image */}
//                          <div className="text-xs w-28">
//                           <div className="font-medium mb-1">Geographic</div>
//                           {r.GeographicImage ? (
//                             <img
...
//                             <span>—</span> 
//                           )}
//                           <div className="mt-1 truncate">{fileLabel(r.GeographicImage)}</div>
//                         </div>
...
//                         <Link to={`/WaterProjectUpdate/${r._id}`}><button  className="inline-flex items-center rounded-lg border border-sky-600 text-sky-700 px-3 py-1.5 hover:bg-sky-50">Update</button></Link>
...
export default function WaterProjectTable() {
  ...
  // - Strips /r2/, document/, allimages/, uploads/ prefixes
  ...
  const toImgUrl = (val) => {
    ...
    if (lower.includes("document/")) {
      s = s.slice(lower.indexOf("document/") + "document/".length);
    } else if (lower.includes("allimages/")) {
      s = s.slice(lower.indexOf("allimages/") + "allimages/".length);
    } else if (lower.includes("uploads/")) {
      s = s.slice(lower.indexOf("uploads/"));
    }
  ...
  const onImgError = (label, raw, tried) => (e) => {
    console.error(`EnergyTable image failed (${label})`, { raw, tried });
    ...
    if (typeof val === "object") {
      ...
      return (
        <div className="max-w-xs">
          <div className="font-medium">{val.title ?? "Achievement"}</div>
          {val.detail && (
            <div className="text-xs text-slate-600 mt-1">{val.detail}</div>
          )}
          {!Number.isNaN(pct) && (
            <div className="mt-2">
              <div className="text-xs font-semibold">{pct}%</div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                ...
                rows.map((r) => {
                  const coverURL = toImgUrl(r.coverImage);
                  const objectiveURL = toImgUrl(r.objectiveImage);
                  const geoURL = toImgUrl(r.GeographicImage);
                  ...
                  return (
                    <tr key={r._id} className="align-top">
                      ...
                      <td className="px-3 py-3">
                        <div className="line-clamp-3 max-w-xs">{r.desc || "—"}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="line-clamp-3 max-w-xs">{r.overview || "—"}</div>
                      </td>
                      <td className="px-3 py-3">
                        <div className="line-clamp-3 max-w-xs">{r.geogrpahic || "—"}</div>
                      </td>
                      <td className="px-3 py-3">{r.componentTitle || "—"}</td>
                      ...
                      <td className="px-3 py-3">{renderAchievementsCell(r.achievements)}</td>
                          ...
                          <div className="text-xs w-28">
                            ...
                            {r.coverImage ? (
                              <img
                                src={coverURL}
                                alt="cover"
                                onError={onImgError("cover", r.coverImage, coverURL)}
                                className="w-28 h-20 object-cover rounded border"
                              />
                            ) : (
                              <span>—</span>
                          ...
                          <div className="text-xs w-28">
                            ...
                            {r.objectiveImage ? (
                              <img
                                src={objectiveURL}
                                alt="objective"
                                onError={onImgError("objective", r.objectiveImage, objectiveURL)}
                                className="w-28 h-20 object-cover rounded border"
                              />
                            ) : (
                              <span>—</span>
                          ...
                          <div className="text-xs w-28">
                            ...
                            {r.GeographicImage ? (
                              <img
                                src={geoURL}
                                alt="geographic"
                                onError={onImgError("geographic", r.GeographicImage, geoURL)}
                                className="w-28 h-20 object-cover rounded border"
                              />
                            ) : (
                              <span>—</span>
                              ...
                              <div key={label} className="text-xs">
                                ...
                                {val ? (
                                  <img
                                    src={u}
                                    alt={label}
                                    onError={onImgError(label, val, u)}
                                    className="w-20 h-16 object-cover rounded border"
                                  />
                                ) : (
                                  <span>—</span>
                                )}
                                <div className="mt-1 truncate max-w-[5rem]">
                                  {fileLabel(val)}
                                </div>
                              </div>
                            );
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center rounded-lg border border-rose-500 text-rose-600 px-3 py-1.5 hover:bg-rose-50"
                          >
                            Delete
                          </button>
                        </div>
