...
//   },[])
  const handlWatereeReadData = () => {
    axios
      .get("https://moewr-backend.onrender.com/readStageProjectWater")
      .then((res) => {
        wsetdata(Array.isArray(res.data) ? res.data : []);
      })
      .catch((err) => {
...
<section id="projects" className="bg-white/70 border-y">
      <div className="max-w-7xl mx-auto px-4 py-16 lg:py-20">
        ...
        {loading ? (
          ...
          <p className="mt-8 text-slate-600">No projects found.</p>
        ) : (
          <div className="mt-10 grid gap-8 md:grid-cols-2 lg:grid-cols-2">
            {wData.map((item) => (
              <article
              ...
              >
                ...
                <div className="mt-4 flex justify-between items-center gap-2">
                  <Link
                    ...
                    className="inline-block px-3 py-2 rounded-md text-white bg-[#2FA8E1] hover:bg-[#0A7FB8]"
                  >
                    Read More
                  </Link>

                  <span className="inline-block px-3 py-2 rounded-md text-black border border-black bg-white hover:bg-[#0A7FB8] hover:text-white">
                    {item.projectStage}
                  </span>
                </div>
              </article>
