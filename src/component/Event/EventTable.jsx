...
export default function EventTable() {
  ...
  // Extract a path (string, multer file-ish object, or already a URL)
  const extractPath = (v) => {
    if (!v) return "";
  ...
  // Build a public R2 URL from whatever is stored
  const resolveImgSrc = (p) => {
    if (!p) return "";
    let s = String(p);
    ...
    // If full disk path exists, cut from known folders
    const docIdx = s.toLowerCase().indexOf("document/");
    if (docIdx !== -1) s = s.slice(docIdx + "document/".length);

    const uploadsIdx = s.toLowerCase().indexOf("uploads/");
    if (uploadsIdx !== -1 && docIdx === -1) s = s.slice(uploadsIdx);

    // Remove Express/static prefixes or stray leading slashes
    s = s
      .replace(/^\/?allimages\/?/i, "")
      .replace(/^\/?document\/?/i, "")
      .replace(/^\/+/, "");

    // If the DB saved "/r2/event/cover/xxx.jpg" → strip "/r2/"
    s = s.replace(/^\/?r2\//i, "");

    // Final public R2 URL
    const url = `${ASSET_BASE}/${s}`;
    return url;
  };
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      // Debug a couple of rows to ensure paths resolve
      arr.slice(0, 3).forEach((ev) =>
        console.log("EventTable image debug:", {
          id: ev._id,
          rawCover: ev.coverImage,
          coverUrl: resolveImgSrc(ev.coverImage),
        })
      );
                      ...
                      <td className="px-4 py-2 border">
                        {ev.coverImage ? (
                          <img
                            ...
                            onClick={() => window.open(resolveImgSrc(ev.coverImage), "_blank")}
                            onError={(e) => {
                              console.error("Cover failed:", {
                                id: ev._id,
                                raw: ev.coverImage,
                                tried: resolveImgSrc(ev.coverImage),
                              });
                      ...
                      <td className="px-4 py-2 border">
                        {ev.objectiveImage ? (
                          <img
                            ...
                            className="h-14 w-24 rounded object-cover border cursor-pointer hover:opacity-80"
                            onClick={() =>
                              window.open(resolveImgSrc(ev.objectiveImage), "_blank")
                            }
                            onError={(e) => {
                              console.error("Objective failed:", {
                                id: ev._id,
                                raw: ev.objectiveImage,
                                tried: resolveImgSrc(ev.objectiveImage),
                              });
                              e.currentTarget.style.display = "none";
                            }}
                          />
