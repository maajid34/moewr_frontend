...
// If your router is NOT under /api, keep it as below. If it IS under /api, change to `${API_BASE}/api/ReadAssess/DocumentFile`
...
export default function PlanningPolicy() {
    ...
    (async () => {
      try {
        ...
        console.error("Assessments fetch error:", {
          message: e?.message, code: e?.code, status: e?.response?.status, data: e?.response?.data, url: LIST_URL,
        });
  ...
  // Optional: show only items that look like assessments/studies
  const studies = useMemo(() => {
    ...
    return list.filter(d => {
      ...
      return c.includes("assessment") || c.includes("study") || t.includes("assessment") || t.includes("study") || ds.includes("assessment") || ds.includes("study");
    });
  }, [docs]);
